...
## API call ----
# Define credentials from config
email <- username
password <- password
...
df %<>%
  mutate(Activein2025 =
           ifelse(Startdate <= "2025-12-31" &
                    Currentenddate >= "2025-01-01", "Yes", "No"))
df %<>%
...
## Filters ----
df %<>% filter(Typeofinvestment != "AF")
df %<>% filter(Country != "Global" | Country != "Regional")
